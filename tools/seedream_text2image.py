...
class SeedreamText2ImageTool(Tool):
    ...
    def _invoke(self, tool_parameters: dict[str, Any]) -> Generator[ToolInvokeMessage]:
        """Seedream 文生图（/v1/images/generations）。
        ...
        logger.info(
            f"[Seedream Text2Image] 请求生成图片，model={model}, prompt_len={len(final_prompt)}"
        )

        try:
            response = requests.post(api_url, headers=headers, json=payload, timeout=300)
            logger.info(f"[Seedream Text2Image] 响应状态: {response.status_code}")

            if not response.ok:
                logger.error(f"[Seedream Text2Image] 错误响应: {response.text}")
            ...
            logger.error(f"[Seedream Text2Image] 网络异常: {str(e)}")
            raise Exception(str(e))
        except Exception as e:
            logger.error(f"[Seedream Text2Image] 异常: {str(e)}")
            raise
