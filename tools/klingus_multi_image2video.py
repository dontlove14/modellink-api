...
class KlingusMultiImage2VideoTool(Tool):
    def _invoke(self, tool_parameters: dict[str, Any]) -> Generator[ToolInvokeMessage]:
        ...
        try:
            ...
            image_list_str = tool_parameters.get('image_list')
            ...
            image_list_str = process_param(image_list_str)
            ...
            image_list = []
            if image_list_str:
                image_list = [img.strip() for img in image_list_str.split(',') if img.strip()]
            
            logger.info(f'[Klingus MultiImage2Video] 开始生成视频，模型: {model_name}，参考图数量: {len(image_list)}')
            ...
            logger.info(f'[Klingus MultiImage2Video] 请求数据: {request_data}')
            
            # 发送请求
            api_url = f"{host}/klingus/v1/videos/multi-image2video"
            headers = {
