...
class KlingusText2VideoTool(Tool):
    def _invoke(self, tool_parameters: dict[str, Any]) -> Generator[ToolInvokeMessage]:
        """Klingus 文生视频生成工具

        参数:
            tool_parameters: 包含 api_key、model_name、prompt 等参数
        ...
        try:
            ...
            logger.info(f'[Klingus Text2Video] 开始生成视频，模型: {model_name}')
            ...
            logger.info(f'[Klingus Text2Video] 请求数据: {request_data}')
            
            # 发送请求
            api_url = f"{host}/klingus/v1/videos/text2video"
            headers = {
            ...
            logger.info(f'[Klingus Text2Video] 响应状态: {response.status_code}')
            
            if not response.ok:
                error_text = response.text
                logger.error(f'[Klingus Text2Video] 错误响应: {error_text}')
                response.raise_for_status()
            
            result = response.json()
            logger.info(f'[Klingus Text2Video] 请求成功，任务 ID: {result.get("data", {}).get("task_id")}')
            ...
            logger.error(f'[Klingus Text2Video] 网络异常: {str(e)}')
            raise Exception(str(e))
        except Exception as e:
            logger.error(f'[Klingus Text2Video] 异常: {str(e)}')
            raise
